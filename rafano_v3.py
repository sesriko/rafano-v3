...
def get_daily_reference_date(now=None):
    """
    LOGIC FINAL SESUAI REQUEST:
    - Market OPEN (09:00-15:50) -> daily = kemarin (previous trading day)
    - Market CLOSED & jam < 18:00 -> daily = kemarin (broksum belum final)
    - Market CLOSED & jam >= 18:00 -> daily = hari ini (broksum final)
    ...
    if market_open:
        ...
        return prev, True, f"Market OPEN {now.strftime('%H:%M')} -> Daily = kemarin {prev.strftime('%d/%m/%Y')} (hari ini belum settle)"
    else:
        if now.time() < cutoff:
            prev = get_previous_trading_day(today)
            return prev, True, f"Market CLOSED {now.strftime('%H:%M')} <18:00 -> Daily = kemarin {prev.strftime('%d/%m/%Y')} (broksum belum final)"
        else:
            return last_today, False, f"Market CLOSED {now.strftime('%H:%M')} >=18:00 -> Daily = hari ini {last_today.strftime('%d/%m/%Y')} (broksum final)"
...
def format_timeframe_label(tf):
    m={"1m":"1 Menit","1min":"1 Menit","5m":"5 Menit","5min":"5 Menit","15m":"15 Menit","15min":"15 Menit","30m":"30 Menit","30min":"30 Menit","1h":"1 Jam","60m":"1 Jam","1hour":"1 Jam","4h":"4 Jam","4hour":"4 Jam","1d":"Daily","daily":"Daily","1w":"Weekly","weekly":"Weekly","1mo":"Monthly","1M":"Monthly","monthly":"Monthly"}
...
# ========== SELL BROKER REAL MODULE - FIX DARI DATA WEB BUMI ==========
def parse_broker_summary_real_web(raw_brokers):
    """Parse format web stockarjum: B.val, S.val, B.avg, S.avg, B.lot, S.lot"""
    parsed=[]
    for b in raw_brokers:
        code=(b.get('broker_code') or b.get('code') or '??').upper()
        b_val=float(b.get('B.val') or b.get('bval') or b.get('buy_value') or 0)
        s_val=float(b.get('S.val') or b.get('sval') or b.get('sell_value') or 0)
        b_lot=float(b.get('B.lot') or b.get('bvol') or 0)
        s_lot=float(b.get('S.lot') or b.get('svol') or 0)
        b_avg=float(b.get('B.avg') or b.get('bavg') or 0)
        s_avg=float(b.get('S.avg') or b.get('savg') or 0)
        n_val=float(b.get('nval') or b_val - s_val)
        parsed.append({
            "broker_code":code,"buy_value":b_val,"sell_value":s_val,
            "buy_volume":b_lot,"sell_volume":s_lot,
            "buy_avg":b_avg,"sell_avg":s_avg,
            "net_value":n_val,"avg_price":b_avg if n_val>0 else s_avg
        })
    return parsed

def get_top_buyers_real(brokers, top=3):
    buyers=[b for b in brokers if float(b.get('net_value',0))>0]
    return sorted(buyers, key=lambda x: float(x.get('net_value',0)), reverse=True)[:top]

def get_top_sellers_real(brokers, top=3):
    sellers=[b for b in brokers if float(b.get('net_value',0))<0]
    return sorted(sellers, key=lambda x: float(x.get('net_value',0)))[:top]

def format_top_brokers_sell(brokers, top=3):
    if not brokers: return "-"
    parts=[]
    for b in brokers[:top]:
        code=b.get('broker_code','??')
        s_val=float(b.get('sell_value',0) or abs(b.get('net_value',0)))
        s_avg=float(b.get('sell_avg',0) or b.get('avg_price',0))
        if abs(s_val)>=1e9: txt=f"{s_val/1e9:.1f}B"
        elif abs(s_val)>=1e6: txt=f"{s_val/1e6:.0f}M"
        else: txt=f"{s_val:.0f}"
        parts.append(f"{code} {txt} ({s_avg:.0f})")
    return ", ".join(parts) if parts else "-"

def format_top_brokers_buy(brokers, top=3):
    if not brokers: return "-"
    parts=[]
    for b in brokers[:top]:
        code=b.get('broker_code','??')
        b_val=float(b.get('buy_value',0) or b.get('net_value',0))
        b_avg=float(b.get('buy_avg',0) or b.get('avg_price',0))
        if abs(b_val)>=1e9: txt=f"{b_val/1e9:.1f}B"
        elif abs(b_val)>=1e6: txt=f"{b_val/1e6:.0f}M"
        else: txt=f"{b_val:.0f}"
        parts.append(f"{code} {txt} ({b_avg:.0f})")
    return ", ".join(parts) if parts else "-"

def analyze_sell_pressure(brokers):
    """Deteksi heavy seller seperti AK -132B di BUMI"""
    if not brokers: return None
    sellers=get_top_sellers_real(brokers,5)
    buyers=get_top_buyers_real(brokers,5)
    total_buy=sum(float(b.get('buy_value',0)) for b in brokers)
    total_sell=sum(float(b.get('sell_value',0)) for b in brokers)
    if not sellers: return None
    heavy=sellers[0]
    heavy_ratio=abs(float(heavy.get('net_value',0)))/ (total_buy+1) if total_buy>0 else 0
    return {
        "heavy_seller": heavy,
        "heavy_ratio": heavy_ratio,
        "sell_dominance": total_sell > total_buy,
        "top_sellers": sellers,
        "top_buyers": buyers
    }
...
def build_professional_caption_with_sell(symbol, last_close, flow_multi, trading_plan, timeframe):
    ...
    tf_desc=get_tf_label(timeframe)
    # REAL SELL BROKER
    top_buy_str=", ".join([f"{b.code} {fmt(b.net_val)}" for b in flow_d.top_accum[:3]]) if hasattr(flow_d,'top_accum') else "-"
    top_sell_str=", ".join([f"{b.code} {fmt(abs(b.net_val))}" for b in flow_d.top_dist[:3]]) if hasattr(flow_d,'top_dist') else "-"
    # Ambil dari raw brokers untuk avg price
    raw_brokers=flow_d.all_brokers if hasattr(flow_d,'all_brokers') else []
    buy_detail=format_top_brokers_buy([{"broker_code":b.code,"buy_value":b.buy_val,"buy_avg":b.avg_price} for b in flow_d.top_accum[:3]] if hasattr(flow_d,'top_accum') else [],3) if hasattr(flow_d,'top_accum') else top_buy_str
    sell_detail=format_top_brokers_sell([{"broker_code":b.code,"sell_value":abs(b.net_val),"sell_avg":b.avg_price,"net_value":b.net_val} for b in flow_d.top_dist[:3]] if hasattr(flow_d,'top_dist') else [],3) if hasattr(flow_d,'top_dist') else top_sell_str
    ...
    caption+=f"────────────────────────────────\n"
    caption+=f"📊 *FLOW REAL + SELL BROKER (Dari API StockArjum)*\n"
    caption+=f"Daily: Buy {fmt(flow_d.total_buy)} | Sell {fmt(flow_d.total_sell)} | Net {fmt(flow_d.total_net)} | {flow_d.status}\n"
    caption+=f"  └ Top Buy: {buy_detail}\n"
    caption+=f"  └ Top Sell: {sell_detail}\n"
    caption+=f"Weekly: {flow_w.status} Buy {fmt(flow_w.total_buy)} Sell {fmt(flow_w.total_sell)} Net {fmt(flow_w.total_net)} Ratio {flow_w.accum_ratio:.0f}%\n"
    caption+=f"Monthly: {flow_m.status} Buy {fmt(flow_m.total_buy)} Sell {fmt(flow_m.total_sell)} Net {fmt(flow_m.total_net)} Ratio {flow_m.accum_ratio:.0f}%\n"
    # Heavy seller warning seperti BUMI AK -132B
    if hasattr(flow_d,'top_dist') and flow_d.top_dist:
        heavy=flow_d.top_dist[0]
        if abs(heavy.net_val) > flow_d.accum_val*0.5:
            caption+=f"⚠️ Heavy Seller: {heavy.code} {fmt(abs(heavy.net_val))} (>{50}% akum)\n"
    caption+=f"────────────────────────────────\n"
...
def build_professional_caption(symbol, last_close, flow_multi, trading_plan, timeframe):
    try:
        return build_professional_caption_with_sell(symbol, last_close, flow_multi, trading_plan, timeframe)
    except:
        pass
    # fallback old
def build_professional_caption(symbol, last_close, flow_multi, trading_plan, timeframe):
...
def get_broker_multi_tf(symbol, hist_df=None, now_override=None):
    """
    V4 FINAL TIMING + SELL BROKER REAL
    - Daily reference ditentukan oleh get_daily_reference_date()
    - Market OPEN atau <18:00 = pakai kemarin
    - Market >=18:00 = pakai hari ini (final)
    - Weekly = 5 hari bursa dari daily_ref
    - Monthly = 20 hari bursa dari daily_ref
    - Plus Top Buy & Top Sell terpisah dari API StockArjum
    """
    ...
    def _fetch_range(d_from, d_to):
        # Coba broker-summary dengan date range
        for p in [{"broker_limit":20,"flow":"all"},{"flow":"all"},{}]:
            date_params={}
            if d_from and d_to:
                def fmt(d): return d.strftime('%d/%m/%Y')
                date_params={"from":fmt(d_from),"to":fmt(d_to),"date_from":fmt(d_from),"date_to":fmt(d_to),"dari":fmt(d_from),"sampai":fmt(d_to)}
            params={**p, **date_params}
            data=arjum_get(f"/broker-summary/{symbol}", params=params, use_cache=False)
            if data and isinstance(data, dict):
                raw=data.get('brokers') or data.get('data') or []
                if raw:
                    # parse dengan field B.val S.val
                    parsed=[]
                    for b in raw:
                        code=(b.get('broker_code') or b.get('code') or '??').upper()
                        ...
                        nval=float(b.get('nval') or bval-sval)
                        parsed.append({"broker_code":code,"buy_value":bval,"sell_value":sval,"buy_avg":bavg,"sell_avg":savg,"avg_price":bavg if nval>0 else savg,"net_value":nval})
                    if parsed:
                        return parsed
        return []
    
    brokers_d = _fetch_range(daily_ref_date, daily_ref_date)
    brokers_5d = _fetch_range(weekly_from, daily_ref_date)
    brokers_20d = _fetch_range(monthly_from, daily_ref_date)
    
    def calc_status(brokers):
        if not brokers: return 0,0,0,"NEUTRAL","EMPTY"
        buy=sum(float(b.get('buy_value',0)) for b in brokers)
        sell=sum(float(b.get('sell_value',0)) for b in brokers)
        net=sum(float(b.get('net_value',0)) for b in brokers)
        status="AKUM" if net>0 else "DIST" if net<0 else "NEUTRAL"
        return buy,sell,net,status,"API_SUMMARY_RANGE"
    
    buy_d,sell_d,net_d,status_d,src_d=calc_status(brokers_d)
    buy_5d,sell_5d,net_5d,status_5d,src_5d=calc_status(brokers_5d)
    buy_20d,sell_20d,net_20d,status_20d,src_20d=calc_status(brokers_20d)
    
    def get_top_buy(brokers, top=3):
        buyers=[b for b in brokers if float(b.get('net_value',0))>0]
        return sorted(buyers, key=lambda x: float(x.get('net_value',0)), reverse=True)[:top]
    def get_top_sell(brokers, top=3):
        sellers=[b for b in brokers if float(b.get('net_value',0))<0]
        return sorted(sellers, key=lambda x: float(x.get('net_value',0)))[:top]
    ...
    result={
        ...
        "buy_d": buy_d, "sell_d": sell_d, "net_d": net_d, "status_d": status_d, "status": status_d,
        "buy_5d": buy_5d, "sell_5d": sell_5d, "net_5d": net_5d, "status_5d": status_5d,
        "buy_20d": buy_20d, "sell_20d": sell_20d, "net_20d": net_20d, "status_20d": status_20d,
        ...
        "top_buy_d": get_top_buy(brokers_d), "top_sell_d": get_top_sell(brokers_d),
        "top_buy_5d": get_top_buy(brokers_5d), "top_sell_5d": get_top_sell(brokers_5d),
        "top_buy_20d": get_top_buy(brokers_20d), "top_sell_20d": get_top_sell(brokers_20d),
