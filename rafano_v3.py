...
def process_chart_request(chat_id, stock_code, timeframe="1d", extra_info_cache=None):
    ...
    try:
        ...
        if multi:
            ...
            daily_line = f"{st_d} | Buy {format_large_number(buy_d,True)} Sell {format_large_number(sell_d,True)} Net {format_large_number(multi.get('net_d',0),True)} Avg {multi.get('avg_d',0):.0f} | {top_d_str}"
            weekly_line = f"{st_5d} | Buy {format_large_number(buy_5d,True)} Sell {format_large_number(sell_5d,True)} Net {format_large_number(multi.get('net_5d',0),True)} Avg {multi.get('avg_5d',0):.0f} | {top_5d_str}"
            monthly_line = f"{st_20d} | Buy {format_large_number(buy_20d,True)} Sell {format_large_number(sell_20d,True)} Net {format_large_number(multi.get('net_20d',0),True)} Avg {multi.get('avg_20d',0):.0f} | {top_20d_str}"
        else:
